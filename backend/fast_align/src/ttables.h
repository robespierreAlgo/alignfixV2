...
//  typedef std::unordered_map<unsigned, double> Word2Double;
  ...
  void NormalizeVB(const double alpha, size_t num_threads) {
    ttable.swap(counts);
...
//     }
    parallel_for(num_threads, 0, ttable.size(), [&](size_t i, size_t thread_id) {
      double tot = 0;
      Word2Double& cpd = ttable[i];
      for (Word2Double::iterator it = cpd.begin(); it != cpd.end(); ++it)
        tot += it->second + alpha;
    ...
    ClearCounts(num_threads);
    probs_initialized_ = true;
  }

  void Normalize(size_t num_threads) {
    ttable.swap(counts);
// #pragma omp parallel for schedule(dynamic)
//     for (unsigned i = 0; i < ttable.size(); ++i) {
//       double tot = 0;
...
//     }
 ...
 private:
  void ClearCounts(size_t num_threads) {
...
//       }
//     }
    parallel_for(num_threads, 0, counts.size(), [&](size_t i, size_t thread_id) {
      for (auto& cnt : counts[i]) {
        cnt.second = 0.0;
      }
    });
